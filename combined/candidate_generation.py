...
def prune_non_important(nouns, tfidf_words):
    ...
    for noun in nouns:
        ...
        if len(synsets) > 0:
            ...
            if arr[1] not in ["person", "location", "group", "cognition", "time", "state", "feeling", "quantity"] and \
                    arr[0] not in ["adj", "adv", "verb"]:
    ...
    for item in categories:
        if len(item) > 2:
            ...
            if base_form:
                ...
                for noun in nouns:
                    score = wns.word_similarity(base_form, noun,
                                                'wpath')  # "Computing Semantic Similarity of Concepts in Knowledge Graphs."
